...
/**
 * 列出 inputRoot 下 *.graph.yaml 的 graphId。
 * @param {string} inputRoot
 * @param {{ recursive?: boolean, maxDepth?: number, skipDirs?: string[], includeShared?: boolean }} [opts]
 * @returns {string[]} 相对 inputRoot 的 posix id（去 .graph.yaml）；扁平文件为 basename
 */
export function allGraphIds(inputRoot, opts = {}) {
  ...
  /** @type {Map<string, string>} id -> abs path（冲突检测） */
  const found = new Map();
...
/** 解析 check/export 用的 graph.json：优先 shared/，否则根下 */
...
/**
 * @param {string} inputRoot
 * @param {{ outPath?: string|null, recursive?: boolean, generatedAt?: string|null, freezeId?: string }} [opts]
 */
export function exportGraphJson(inputRoot, opts = {}) {
  const recursive = opts.recursive !== false;
  const payload = buildGraphPayload(inputRoot, {
    generatedAt: opts.generatedAt,
    freezeId: opts.freezeId,
    recursive,
  });
...
export function buildGraphPayload(
  inputRoot,
  { generatedAt = null, freezeId = FREEZE_ID, recursive = true } = {},
) {
