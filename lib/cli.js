...
用法:
  ...
  npx @cyning/harness task lint-wiki-delta [--target PATH] [--scope all|active|done] [--json]
...
说明:
  ...
  task lint-wiki-delta  列出缺 wiki_delta 字段的 task（升级迁移清单 · v2.19+ · 缺则 exit 2）
...
async function cmdTaskLintWikiDelta(args) {
  if (args.includes('--help') || args.includes('-h')) {
    console.log(`用法: npx @cyning/harness task lint-wiki-delta [--target PATH] [--scope all|active|done] [--json]

升级后迁移清单（v2.19+ · F-218-01）：扫描 task md 元信息是否含 wiki_delta 字段。
  默认扫描 docs/tasks/{active,done} 与 docs/harness/tasks/{active,done}（递归）
  --scope active|done  仅扫对应布局；缺省 all
  仅查「缺字段」；none/n/a 缺 note、path 不存在仍由 verify/close 处理
  有缺失 → exit 2；全齐 → exit 0
  --json：{ ok, scope, scanned, missing: [{ path, scope }] }
补救：见 USER_GUIDE §6.0b 决策树。
`);
    return;
  }
  ...
  let result;
  try {
    result = lintWikiDeltaMissing(target, { scope: scopeArg || 'all' });
  } catch (e) {
  ...
  if (json) {
    ...
    console.log(`目标: ${target}`);
    console.log(`scope: ${result.scope} · scanned: ${result.scanned} · missing: ${result.missing.length}`);
    if (!result.ok) {
      console.log('缺 wiki_delta 字段的 task:');
      for (const m of result.missing) console.log(`  - ${m.path}`);
      console.log('补救: USER_GUIDE §6.0b · 决策树 n/a|none|path');
    }
    console.log(result.ok ? 'LINT-WIKI-DELTA: PASS' : `LINT-WIKI-DELTA: FAIL · missing ${result.missing.length}`);
  }
