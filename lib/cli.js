...
async function cmdVerify(args) {
  ...
  if (args.includes('--help') || args.includes('-h')) {
    console.log(`用法: npx @cyning/harness verify [--target PATH] [--task FILE | --spec FILE] [--graph] [--json] [--agent-hint] [--workspace-root PATH] [--allow-no-review] [--allow-lint-fail] [--allow-no-spec-review] [--allow-invoke-gap] [--strict-graph-delta]
...
--task：30 前聚合（gate + audit + reviews + lint WARN + pre-30 invoke 硬闸 · v2.17+；缺 40 仍 WARN）· graph_delta 默认 WARN · --allow-no-review / --allow-lint-fail / --allow-invoke-gap / --strict-graph-delta。
...
async function cmdTaskClose(args) {
  if (args.includes('--help') || args.includes('-h')) {
    console.log(`用法: npx @cyning/harness task close --file PATH [--target PATH] [--yes] [--allow-unchecked] [--allow-no-review] [--allow-invoke-gap] [--allow-kpi-gap] [--allow-experience-gap]

受闸归档（方案 A · invoke 留档机械闸 · v2.12+ 多帽集合 · v2.17+ 闭环硬闸）：
  机械校验：invoke hats · 自检结论 · 验收勾选 · slug · 状态 · R1 review · graph_delta · KPI 打分 · experience
