...
--task：必填 · 按 HGM 事件投影该 task 时间线（升序）。
--json：机读 obs_timeline.v1。
--limit N：最多返回 N 条（仍报告全量 event_count）。
--ingest：显式先跑 graph ingest（写盘）；默认仅 WARN 提示，不偷偷写盘。
...
`);
  ...
  rest = r3;
  const json = rest.includes('--json');
  const ingest = rest.includes('--ingest');
  rest = rest.filter((a) => a !== '--json' && a !== '--ingest');

  if (rest.length > 0) {
    const err = new Error(`timeline 未知参数: ${rest.join(' ')}`);
    err.exitCode = 1;
    throw err;
  }
  if (!taskFile) {
