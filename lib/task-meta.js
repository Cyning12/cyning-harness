...
/** 无反引号的表值（graph_delta_note / experience_capture_note 等自由文本） */
const META_FIELD_PLAIN_RE = /\|\s*\*\*([^*]+)\*\*\s*\|\s*([^|\n]+)\|/;
const GATE_ROW_RE = /^\|\s*(?:\*\*)?([^*|]+?)(?:\*\*)?\s*\|\s*([^|]+)\|\s*([^|]+)\|\s*([^|]*)\|/;

/**
 * 从 task Markdown 解析 Harness 元信息表。
 * 优先反引号包裹的值；否则取纯文本单元格（笔记类字段）。
 */
export function parseHarnessMeta(content) {
  ...
  for (const line of section.split('\n')) {
    ...
    const plain = line.match(META_FIELD_PLAIN_RE);
    if (!plain) continue;
...
/**
 * 从 invoke 文件名提取 hat token 集合（v2.12.1+：前缀 hat run）。
 * 兼容：invoke_YYYYMMDD_30_slug.md · invoke_YYYYMMDD_30_40_slug.md · invoke_*_CLOSE_*.md
 * 只消费日期后的**连续前缀** hat token；遇第一个非 hat 段即停（其后为 slug，不误计）。
...
export function extractHatsFromInvokeFilename(fileName) {
  ...
  for (const p of parts) {
    ...
    // 首个非 hat 段起视为 slug · 停止扫描（防 slug 含 10/40 误计）
    break;
  }
...
/** 30 开工前须具备的 invoke hats（与 required 求交）· v2.14+ */
const PRE30_INVOKE_HATS = new Set(['10', '20', '00']);
...
/**
 * verify 前 30：pre-30 hats（required ∩ {10,20,00}）缺口硬闸（v2.14+）。
 * preRequired 为空（如 minimal）→ skipped/ok；缺目录/空目录且 preRequired 非空 → 视为全缺。
 * 不因缺 40/30 挡 may_start_30。
 * @returns {{
 *   preRequired: string[],
 *   preMissing: string[],
 *   postMissing: string[],
 ...
 * }}
 */
export function evaluatePre30InvokeHats(meta, invokeDir) {
  const full = evaluateInvokeHatsRetention(meta, invokeDir);
  const preRequired = full.required.filter((h) => PRE30_INVOKE_HATS.has(h));
  const postMissing = full.missing.filter((h) => !PRE30_INVOKE_HATS.has(h));
  ...
  const emptyOrMissingDir = full.missingDir || full.files.length === 0;
  const coveredSet = new Set(full.covered);
  const preMissing = emptyOrMissingDir
    ? [...preRequired]
    : preRequired.filter((h) => !coveredSet.has(h));
