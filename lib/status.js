...
import { summarizeTaskHgm } from './obs-hgm.js';
...
export function buildTaskStatus(target, taskFile, options = {}) {
  ...
  if (check) {
    if (!review.found) {
      checkReasons.push('missing R<n> review');
    }
    if (!gateEval.may_start_30) {
      checkReasons.push(
        `may_start_30=false · ${gateEval.blocked_reason || blockers.join('; ') || 'blocked'}`,
      );
    }
    if (checkReasons.length > 0) {
      checkFailed = true;
      for (const r of checkReasons) {
        warnings.push(`FAIL: status --check · ${r}`);
      }
    } else {
  ...
  return { payload, warnings, checkFailed, checkReasons };
}
...
/**
 * 下一动作提示：按生命周期阶段，避免「已过 30 仍提示开 30」。
 */
export function buildNextHint({
  ...
  if (closeFound || st === 'done' || st === 'completed') {
    return '已关账 · 可用 status/timeline 复盘；下一棒见 Epic';
  }

  const post30 = lastHat === '30' || lastHat === '40' || lastHat === '50' || lastHat === 'CLOSE';
  if (post30 && mayStart30 && reviewFound) {
    if (lastHat === 'CLOSE') {
      return 'CLOSE invoke 已在 · 确认归档 active→done';
    }
    if (lastHat === '40' || lastHat === '50') {
      return '自检/复检已过 · 写 CLOSE 审查并 task close 归档';
    }
    // lastHat === '30'
    if (!verifyOk) {
      return '30 已执行 · 先消除 verify_preview BLOCK，再跑正式 verify / 40';
    }
    return '30 已执行 · 跑正式 verify 后做 40 自检，再 CLOSE（status 不替代 verify）';
  }
  ...
  return '闸已齐 · 跑 harness verify --task … 后开 30（status 不替代 verify）';
}
