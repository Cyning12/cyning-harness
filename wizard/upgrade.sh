...
非交互（脚本 / CI）:
  ...
  ./wizard/upgrade.sh --target /path/to/project --yes --force   # 跳过 S5 git-clean

选项:
  --target PATH    业务仓根（交互模式下可省略，运行时会提示输入）
  --ide LIST       更新 IDE 勾选：cursor,claude,agents
  --yes            预览 plan 后不再确认，直接 apply
  --force          sync apply 跳过 S5 git-clean 检查
...
[[ "$SYNC_FORCE" -eq 1 ]] && export HARNESS_SYNC_FORCE=1
...
echo ""
echo "日后可运行: npx @cyning/harness upgrade --target $TARGET"
echo "或维护者: $SCRIPT_DIR/upgrade.sh --target $TARGET"
