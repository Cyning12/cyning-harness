...
|---|---|---|---|---|---|
| **P0** | **G1 · task md 结构 lint** | 高（下游全部闸的输入假设） | 低（解析与 M5 共享） | `harness task lint --file PATH`：必填节（元信息/状态行/验收标准/failure_paths/自检结论/思考轮槽）+ 占位符探测 | 10 产出时 + verify（30 前） |
...
| **P1** | **G3 · 文本纪律 grep 闸** | 中（泄漏本机路径/预写 approved 属低级但真实错误） | 极低（正则） | 并入 `task lint`：禁 `(/Users/|/home/|C:\\)`；lint-done 或独立 invoke-lint：禁 invoke 文件含 `HG-AUDIT-R1 approved` 预写句 | 同 G1 |
