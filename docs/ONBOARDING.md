...
一页操作序（升级 → 扫描 → 补字段 → 可选 CI / topics）：[`RUNBOOK_upgrade_wiki_delta_v1_zh.md`](./RUNBOOK_upgrade_wiki_delta_v1_zh.md)（v2.19.1+；**v2.21+** 快速路径/pin；**v2.22+** overlay；**v2.22.1+** 首次接入 overlay **强制序 §1.2.1**——先 commit 契约再 upgrade）。  
